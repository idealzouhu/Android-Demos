...
热词用于提升专有名词、生僻词等特定词汇的识别率（上下文偏置）。将 `hotwords.txt` 放在 `app/src/main/assets/` 下，每行一个热词，支持中文、英文或中英混合（与模型建模单元一致）。仅 **Transducer 模型** 在 **modified_beam_search** 解码方式下支持热词。

- 热词文件路径在配置中为 `hotwordsFile = "hotwords.txt"`（相对 assets）
- 热词权重 `hotwordsScore`（默认 1.5，当前示例为 2.0）
- 中英双语模型需在 `OnlineModelConfig` 中设置 `modelingUnit = "cjkchar+bpe"`、`bpeVocab = "模型目录/bpe.vocab"`
- 详细说明见 [sherpa-onnx 热词文档](https://k2-fsa.github.io/sherpa/onnx/hotwords/index.html)
...
注意，如果要修改的话，建议修改 getEndpointConfig() 函数返回的 EndpointConfig，而不是直接修改 EndpointConfig 对象。
